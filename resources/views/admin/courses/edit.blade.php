            ...
            <form action="{{ route("admin.courses.update", [$course->id]) }}" method="POST"
                ...
                <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                    ...
                    <textarea id="description" name="description"
                              class="form-control ">{{ old('description', isset($course) ? $course->description : '') }}</textarea>
                ...
                <div class="form-group {{ $errors->has('disciplines') ? 'has-error' : '' }}">
                    ...
                    <select name="disciplines[]" id="disciplines" class="form-control select2" multiple="multiple">
                        @foreach($disciplines as $id => $disciplines)
                            <option
                                value="{{ $id }}" {{ (in_array($id, old('disciplines', [])) || isset($course) && $course->disciplines->contains($id)) ? 'selected' : '' }}>{{ $disciplines }}</option>
                        @endforeach
                    </select>
...
@section('scripts')
    ...
    <script>
        Dropzone.options.bookDropzone = {
            ...
            init: function () {
                @if(isset($course) && $course->getFirstMedia('books'))
                var file = {!! json_encode($course->getFirstMedia('books')) !!};
                var mockFile = { name: file.name, size: file.size };
                this.emit("addedfile", mockFile);
                this.emit("complete", mockFile);
                $('form').append('<input type="hidden" name="book" value="' + file.id + '">');
                @endif
            }
        };
    </script>
